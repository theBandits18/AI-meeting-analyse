...
const countPhraseMatches = (text, phrase) => {
  const matcher = new RegExp(`\\b${escapeRegExp(phrase)}\\b`, 'g')
  return text.match(matcher)?.length ?? 0
}

export function analyzeTranscript(transcript, totalCost) {
  const normalizedText = String(transcript ?? '').toLowerCase()
  ...
  const questionCount = String(transcript ?? '').match(/\?/g)?.length ?? 0
  ...
  const costPerDecision =
    decisionPhraseCount > 0 ? safeCost / decisionPhraseCount : safeCost
